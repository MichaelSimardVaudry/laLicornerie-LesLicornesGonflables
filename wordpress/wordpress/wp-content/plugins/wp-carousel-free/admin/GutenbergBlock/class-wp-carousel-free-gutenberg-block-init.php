...
if ( ! class_exists( 'WP_Carousel_Free_Gutenberg_Block_Init' ) ) {
	...
	class WP_Carousel_Free_Gutenberg_Block_Init {
		...
		public function sp_wp_carousel_free_render_shortcode( $attributes ) {
			...
			return $style . '<div id="' . uniqid() . '" ' . $class_name . ' ><a href="' . $edit_page_link . '" target="_blank" class="sp_wp_carousel_block_edit_button">Edit View</a>' . do_shortcode( '[sp_wpcarousel id="' . sanitize_text_field( $attributes['shortcode'] ) . '"]' ) . '</div>';
		}
	}
}
